...
class SREWorkflow:
    ...
    async def _query_metrics(self, service: str, namespace: str) -> ToolResult:
        """查詢 Prometheus 指標"""
        try:
            logger.info(f"📊 開始查詢 {service} 的 Prometheus 指標...")
            
            # 執行四大黃金指標查詢
            results = await self.prometheus_tool.query_golden_signals(
                service_name=service,
                namespace=namespace,
                duration="5m"
            )
            
            logger.info(f"✅ Prometheus 指標查詢完成: {service}")
    ...
    async def _query_logs(self, service: str, namespace: str) -> ToolResult:
        """查詢 Loki 日誌"""
        try:
            logger.info(f"📜 開始查詢 {service} 的 Loki 日誌...")
            
            # 根據服務名稱查詢日誌，特別關注錯誤日誌
            results = await self.loki_tool.query_logs_by_service(
                service_name=service,
                namespace=namespace,
                level="error",
                duration="5m"
            ...
            logger.info(f"✅ Loki 日誌查詢完成: {service}")
            return ToolResult(success=True, data=results)
    ...
    async def _query_audit_logs(self, service: str) -> ToolResult:
        """查詢審計日誌"""
        try:
            logger.info(f"🛂 開始查詢 {service} 的 Control Plane 審計日誌...")
            
            # 查詢指定服務的審計日誌
            results = await self.control_plane_tool.get_audit_logs(
                service_name=service,
                limit=5 
            )
            
            logger.info(f"✅ Control Plane 審計日誌查詢完成: {service}")
            return ToolResult(success=True, data={"recent_changes": results})
            
        except Exception as e:
            logger.error(f"❌ Control Plane 審計日誌查詢失敗: {e}")
            return ToolResult(
                success=False,
    ...
    def _analyze_deployment_results(self, results: Dict[str, ToolResult], request: SRERequest) -> Dict[str, Any]:
        ...
        if "loki" in results and results["loki"].success:
            ...
            # 使用新的 'critical_indicators' key
            if logs.get("critical_indicators"):
                findings.append(Finding(
                    source="Loki",
                    severity=SeverityLevel.P0,
                    data={"errors": logs["critical_indicators"]},
                    timestamp=datetime.now(timezone.utc)
                ))
                issues.extend(logs["critical_indicators"])
